...
All three are generated, so none can drift from the schedule:
...
from schedule import ROOT, Row, Schedule, load  # noqa: E402
...
# lectures-and-labs/README.md inside the week folders' parent.
FROM_ROOT = (f"{ROOT.as_posix()}/", "mcq/README.md")
FROM_INDEX = ("", "../mcq/README.md")
...
def table_row(row: Row, current: Row | None, links: tuple[str, str] = FROM_ROOT) -> str:
    ...
    if row.mcq:
        ...
        return (f"| {week} | **{row.assessment}**{note} | "
                f"[details]({folder}/README.md) · [brief]({brief}) | — |")
    return f"| {week} | {row.notes or 'Reading week'} | [details]({folder}/README.md) | — |"
...
def main() -> None:
    ...
    index = replace_table(index, render_table(sched, current, FROM_INDEX), INDEX)
